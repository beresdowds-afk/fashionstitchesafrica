...
BEGIN
  checks := jsonb_build_object(
    ...
    'webauthn_credentials_rls', COALESCE((
      SELECT relrowsecurity FROM pg_class c
      JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE n.nspname = 'public' AND c.relname = 'webauthn_credentials'
    ), false),
    'webauthn_challenges_rls', COALESCE((
      SELECT relrowsecurity FROM pg_class c
      JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE n.nspname = 'public' AND c.relname = 'webauthn_challenges'
    ), false),
    'webauthn_backup_codes_rls', COALESCE((
      SELECT relrowsecurity FROM pg_class c
      JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE n.nspname = 'public' AND c.relname = 'webauthn_backup_codes'
    ), false),
    ...
    'credential_updated_at_trigger', EXISTS (
      SELECT 1 FROM pg_trigger t
      JOIN pg_class c ON c.oid = t.tgrelid
      JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE n.nspname = 'public'
        AND c.relname = 'webauthn_credentials'
        AND t.tgname = 'trg_webauthn_credentials_updated_at'
        AND NOT t.tgisinternal
        AND t.tgenabled <> 'D'
    )
  );
  ...
  RETURN jsonb_build_object('ok', COALESCE(healthy, false), 'checks', checks);
END;
$$;
