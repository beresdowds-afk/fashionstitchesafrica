...
Create a single ecosystem identity system where a user registering on **their approved PWA** automatically receives a FYSORA FASHN ecosystem account.
...
- Shared session management across ecosystem domain and subdomains (i.e. fs-africa.org.ng and `*.fs-africa.org.ng`).
...
- CSRF protection (e.g., CSRF tokens, SameSite cookies).- `PUT /api/auth/password` – updates password (authenticated).
- `GET /api/auth/profile` – returns user profile.
- `PUT /api/auth/profile` – updates profile (email, phone, etc.) with validation.
